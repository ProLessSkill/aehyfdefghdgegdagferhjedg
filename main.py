...
async def funhelp(ctx):
    ...
    embed.add_field(name="azcount", value = "**Description:** Shows a count of alphabet. My cream hurts.\n**Permission Required** None\n**Arguments:** `None`\n```azcount```", inline=False)
        ...
        if ctx.message.author.id == "393069508027351051":
            ...
            for extension in [f.replace('.py', "") for f in listdir("cogs") if isfile(join("cogs", f))]:
                try:
                    ...
                    await bot.say("⛔️ | Failed to load mog {}".format(extension))
                    traceback.print_exc()
        else:
...
async def commandhelp(ctx):
    ...
    embed.add_field(name="bannnnnn", value = "**Description:** Spams a user with a message of your choice.\n**Permission Required:** Ban Members\n**Arguments:** `Integer`, `Message`\n```m!bannnnn @user [int][message]```", inline=False)
...
async def pineapple(ctx):
    ...
    embed.set_footer(text="PineappleBot | Created by ProLessSkill")
...
# Adds a reaction if a message contains all letters of the alphabet
    ...
    if message.content.lower().startswith('m!azcount'):
	    f = open('az.txt', 'r')
	    count = f.read()
	    f.close()
    ...
    if message.content.lower().startswith('m!shib'):
	    msg = await bot.send_message(message.channel, 'loading...')
	    e = discord.Embed(colour=0xffffff,)
    ...
    for word in contents:
        if word.upper() in chat_filter:
            ...
            await bot.send_message(message.author, "<@%s>: **The thing you just sai triggered my filter, so it was removed.**." % (userID))

# Insert your token here
bot.run("NTA1NzYwNTA3ODc4MzA5OTA4.Dr9tZg.2aSDdrysgF2j-Ii6PTaPOjA5I8Q")
